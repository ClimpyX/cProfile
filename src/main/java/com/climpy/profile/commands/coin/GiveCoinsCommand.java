...
public class GiveCoinsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command label, String s, String[] args) {
        ...
        sender.sendMessage(C.color(user.getRankType().getColor() + t.getName() + " &aoyuncusunun kredi değeri &f" + NumberUtil.formatNumberByDecimal(Double.parseDouble(args[1])) + " &akadar eklendi."));
        t.sendMessage(C.color("&aKredi değerinize &f" + NumberUtil.formatNumberByDecimal(Double.parseDouble(args[1])) + " &akadar eklendi."));
        user.save();
        return true;
    }
}
